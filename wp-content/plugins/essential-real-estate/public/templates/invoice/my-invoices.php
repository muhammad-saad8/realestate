...
<div class="row ere-user-dashboard">
	...
	<div class="col-lg-9 ere-dashboard-content">
		<div class="card ere-card ere-my-invoices">
            <div class="card-header"><h5 class="card-title m-0"><?php echo esc_html__('My Invoices', 'essential-real-estate'); ?></h5></div>
            <div class="card-body">
							...
							<div class="form-group">
								...
								<select class="selectpicker form-control" id="invoice_type" name="invoice_type">
									<option
										...
										echo ' selected' ?>><?php esc_html_e( 'All Invoice Type', 'essential-real-estate' ); ?></option>
									<option
											value="Package" <?php if ( $invoice_type == 'Package' )
										echo ' selected' ?>><?php esc_html_e( 'Package', 'essential-real-estate' ); ?></option>
									<option
											value="Listing" <?php if ( $invoice_type == 'Listing' )
										echo ' selected' ?>><?php esc_html_e( 'Listing', 'essential-real-estate' ); ?></option>
									<option
											value="Upgrade_To_Featured"<?php if ( $invoice_type == 'Upgrade_To_Featured' )
										echo ' selected' ?>><?php esc_html_e( 'Upgrade to Featured', 'essential-real-estate' ); ?></option>
									<option
											value="Listing_With_Featured"<?php if ( $invoice_type == 'Listing_With_Featured' )
										echo ' selected' ?>><?php esc_html_e( 'Listing with Featured', 'essential-real-estate' ); ?></option>
								</select>
							</div>
						</div>
						<div class="col-lg-3 col-sm-6">
							<div class="form-group">
								...
								<select class="selectpicker form-control" id="invoice_status" name="invoice_status">
									<option
										...
										echo ' selected' ?>><?php esc_html_e( 'All Payment Status', 'essential-real-estate' ); ?></option>
									<option
											value="1" <?php if ( $invoice_status == '1' )
										echo ' selected' ?>><?php esc_html_e( 'Paid', 'essential-real-estate' ); ?></option>
									<option
											value="0" <?php if ( $invoice_status == '0' )
										echo ' selected' ?>><?php esc_html_e( 'Not Paid', 'essential-real-estate' ); ?></option>
								</select>
							</div>
						</div>
						<div class="col-lg-2 col-sm-6">
					...
					<table class="table">
						...
						<tbody>
							<?php if ( ! $invoices ) : ?>
								<tr>
									<td colspan="7"
									    data-title="<?php esc_attr_e( 'Results', 'essential-real-estate' ); ?>"><?php esc_html_e( 'You don\'t have any invoices listed.', 'essential-real-estate' ); ?></td>
								</tr>
							<?php else : ?>
									...
									<tr>
										<?php foreach ( $my_invoices_columns as $key => $column ) : ?>
											<td class="<?php echo esc_attr( $key ); ?>"
												...
												elseif ( 'total' === $key ):
													echo wp_kses_post(ere_get_format_money( $invoice_meta['invoice_item_price'] )) ;
