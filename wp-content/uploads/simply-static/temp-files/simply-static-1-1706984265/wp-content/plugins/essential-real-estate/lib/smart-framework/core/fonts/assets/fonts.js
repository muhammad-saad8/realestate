    ...
    GSF_Fonts = {
        ...
        events : function() {
            $('.gsf-reset-active-fonts').on('click',function(event){
                ...
                if (!confirm(GSF_FONT_META_DATA.msgConfirmResetFont)) {
                    return;
                }
                ...
                $.ajax({
                    url: GSF_FONT_META_DATA.ajaxUrl,
                    data: {
                        action: 'gsf_reset_active_font',
                        _nonce: GSF_Fonts._nonce
                    },
                    type: 'post',
        ...
        enqueueFont: function (font) {
            ...
            switch (font.kind) {
                ...
                case 'custom': {
                    url = typeof font.css_url !== 'undefined' ? font.css_url :   GSF_FONT_META_DATA.font_url + font.css_file;
                    break;
                }
            }
            if (url !== '') {
        ...
        addEventListener: function ($container) {
            ...
            $container.find('.gsf-font-item-action-delete').on('click', function (event) {
                event.preventDefault();
                if (!confirm(GSF_FONT_META_DATA.msgConfirmDeleteCustomFont)) {
                    return;
                }
                if (GSF_Fonts._isSubmitting) {
                    return;
        ...
        activeFontAddEventListener: function ($container) {
            ...
            $container.find('.gsf-font-active-item-remove').on('click', function (event) {
                event.preventDefault();
                if (!confirm(GSF_FONT_META_DATA.msgConfirmRemoveActiveFont)) {
                    return;
                }
