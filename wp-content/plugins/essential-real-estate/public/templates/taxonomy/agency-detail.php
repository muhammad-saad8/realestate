	...
	while ( $agent_data->have_posts() ): $agent_data->the_post();
		...
		if ( ! empty( $agent_id ) ) {
			...
			if ( ! empty( $agent_user_id ) ) {
				$author_id_arr[] = $agent_user_id;
			}
		}
	endwhile;
	wp_reset_postdata();
	$agent_id_arr  = join( ',', $agent_id_arr );
	$author_id_arr = join( ',', $author_id_arr );
}
...
<?php endif; ?>
		...
		<div class="agency-info">
			<?php if ( ! empty( $agency_office_number ) ): ?>
				<div class="agency-info-item agency-office-number">
					<i class="fa fa-phone"></i><strong
							class="agency-info-title"><?php esc_html_e( 'Phone', 'essential-real-estate' ) ?>: </strong>
			...
			<?php if ( ! empty( $agency_mobile_number ) ): ?>
				<div class="agency-info-item agency-mobile-number">
					<i class="fa fa-mobile-phone"></i><strong
							class="agency-info-title"><?php esc_html_e( 'Mobile', 'essential-real-estate' ) ?>
						: </strong>
			...
			<?php if ( ! empty( $agency_fax_number ) ): ?>
				<div class="agency-info-item agency-fax-number">
					<i class="fa fa-print"></i><strong
							class="agency-info-title"><?php esc_html_e( 'Fax', 'essential-real-estate' ) ?>: </strong>
			...
			<?php if ( ! empty( $agency_website_url ) ): ?>
				<div class="agency-info-item agency-website">
					<i class="fa fa-external-link-square "></i><strong
							class="agency-info-title"><?php esc_html_e( 'Website', 'essential-real-estate' ) ?>
						: </strong>
					<a href="<?php echo esc_url( $agency_website_url ) ?>"
					   class="agency-info-value"><?php echo esc_url( $agency_website_url ) ?></a>
				</div>
			<?php endif; ?>
			<?php if ( ! empty( $agency_email ) ): ?>
				<div class="agency-info-item agency-email">
					<i class="fa fa-envelope"></i><strong
							class="agency-info-title"><?php esc_html_e( 'Email', 'essential-real-estate' ) ?>: </strong>
	...
	<div class="contact-agency col-md-4 col-sm-12 sm-pd-top-40">
		...
		<form method="post" enctype="multipart/form-data" action="#" id="contact-agent-form">
			...
			<?php if ( ere_enable_captcha( 'contact_agency' ) ) {
	...
	<div class="agency-details-tab ere-tabs">
		<ul class="nav nav-tabs" id="ere-agency-details-tabs">
			...
			<?php if ( $enable_property_of_agency ): ?>
				<li class="nav-item"><a class="nav-link<?php echo (empty($agency_content)) ? ' active' : '' ?>" data-toggle="tab"
				                                                                              href="#agency-properties"><?php esc_html_e( 'Properties', 'essential-real-estate' ); ?></a>
				</li>
			<?php endif; ?>
			<?php if ( ! empty( $agency_map_address['location'] ) ): ?>
				<li class="nav-item">
					<a class="nav-link<?php echo ( empty( $agency_content ) && ! $enable_property_of_agency ) ? ' active' : '' ?>" data-toggle="tab"
					   href="#agency-location"><?php esc_html_e( 'Location', 'essential-real-estate' ); ?></a>
				</li>
			<?php endif; ?>
		</ul>
		<div class="tab-content">
			...
			<?php if ( $enable_property_of_agency ): ?>
				<div id="agency-properties"
				     class="tab-pane fade <?php if ( empty( $agency_content ) ): ?> show active<?php endif; ?>">
					<?php
					...
					$property_of_agency_image_size   = ere_get_option( 'property_of_agency_image_size', ere_get_loop_property_image_size_default() );
					...
					echo ere_do_shortcode( 'ere_property', array(
						'layout_style' => $property_of_agency_layout_style,
						'item_amount'  => $property_of_agency_items_amount,
						'columns'      => $property_of_agency_column_lg,
			...
			<div id="agency-location"
			     class="tab-pane fade <?php if ( empty( $agency_content ) && ! $enable_property_of_agency ): ?> show active<?php endif; ?>">
				<?php
				list( $lat, $lng ) = explode( ',', $agency_map_address['location'] );
				echo ere_do_shortcode( 'ere_property_map', array(
					'map_style'  => 'normal',
					'lat'        => $lat,
					'lng'        => $lng,
					'map_height' => '400px'
